<template>
    <v-main class="d-flex text-center align-center justify-center" style="min-height: 300px;">         
        ...
        <v-container fluid>            
            ...
            <header-pane @scroll-to-next="scrollTo('about')" v-intersect="onIntersectHeader"/>

            <Divider class="my-10 mx-10 text-accent"/>

            <about-pane @scroll-to-next="scrollTo('projects')" v-intersect="onIntersectAbout" />

            <Divider class="my-10 mx-10 text-accent" />

            <projects-pane @scroll-to-next="scrollTo('top')" v-intersect="onIntersectProjects" />

        </v-container>
        
    </v-main>
    
    <v-bottom-navigation class="bg-background" v-model="value" mandatory density="comfortable">

        <v-btn v-show="!userAtTop" variant="text" class="text-accent" color="accent">              
            <span>Sean Harding</span>            
        </v-btn>
        <v-btn v-show="!userAtTop" prepend-icon="mdi-copyright" variant="text" class="text-accent" :stacked="false">
            2024
        </v-btn>
        ...
        <v-btn v-show="!userAtTop" value="top" active-color="accent" @click="scrollTo('top')">
        ...
        <v-btn v-for="item in socials" v-show="!userAtTop" :key="item.name" :href="item.link" target="_blank">
